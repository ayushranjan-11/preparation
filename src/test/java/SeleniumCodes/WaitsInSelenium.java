...
//		waitsInSelenium.ExplicitWaitInSelenium();
	...
	void ExplicitWaitInSelenium() {
		// ExplicitWait is not a universal wait, it is used for specific element/s with
	...
	void FluentWaitInSelenium() {
		...
		FluentWait<WebDriver> fluentWait = new FluentWait<>(driver).withTimeout(Duration.ofSeconds(10))
				.pollingEvery(Duration.ofMillis(500)).withMessage("Element was found successfully")
				.ignoring(Exception.class);

		WebElement webElement = fluentWait.until(new Function<WebDriver, WebElement>() {
			public WebElement apply(WebDriver webDriver) {
				WebElement element = driver.findElement(By.className(className)); // className content is passed in
																					// string as intentionally to let
																					// fluent wait fails
	...
	void BrowserQuit() {
		if (driver != null) {
			...
			if (openedWindowHandles.length > 1) {
				...
				driver.close(); // will close the current tab of the browser and if more than 1 then it will
